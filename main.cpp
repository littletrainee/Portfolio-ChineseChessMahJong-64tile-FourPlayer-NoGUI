...
class PrintAll {
	public:
		...
		void PrintAll_SetNameAndMap(string player1, string player2, string player3, 
				string player4, string wall, string river, map<string, string> tile) {
		...
		void PrintHand(int whitchplayer, vector<string> hand, bool shownumber) {
			// -1 = river, 0 = wall, 1 = player1, 2 = player2, 3 =  player3, 4 = player4
			switch (whitchplayer) { 
				case -1:
					cout << this->river;
					break;
		...
		void PrintAll_Print(Player player1, Player player2,	Player player3, 
												Player player4,	vector<string> wallhand,
...
void ContinueToBookmakerOrChange(int &continuetobookmaker, int winner,
		...
		bool &player4bookmaker, int tempbookmaker, int &gameturn){
...
ReStart:
	...
	while (gameloop) {
		...
		// player4 discard and other player check tsumo, ron or meld
		if (gameturn == 0) {
	...
	if (player3.bookmaker && winnerortempgameturn != 3) {
		...
		ContinueToBookmakerOrChange(continuetobookmaker, winnerortempgameturn, 
				player1.bookmaker, player2.bookmaker, player3.bookmaker, player4.bookmaker, 
				tempbookmaker, gameturn);
		tempbookmaker = 0;
		...
		pa.PrintAll_SetNameAndMap(player1.name, player2.name, player3.name, player4.name, 
				"牌山",	"牌河", tile);
		// cw class set tile
		cw.CheckWin_SetTile(sortby);
		// cm class set tile
